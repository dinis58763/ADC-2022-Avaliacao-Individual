...
@WebServlet("/usermenu" // "/delete"
)

@Path("/login")
@Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")

public class roleUSEResource extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (user != null) {
			...
			if (roleUser.equals("user")) {
				...
				nameUp = user.getString("user_name"); // can't modify name
				emailUp = user.getString("user_email"); // can't modify email
				...
				if (roleUp.equals(role))
					roleUp = role; // can't update role
				else
					response.sendRedirect("/unauthenticated.html");

				Entity newEntity = Entity.newBuilder(userKey).set("user_name", nameUp)
						.set("user_pwd", passwordUp).set("user_pwd_check", passwordCheckUp)
						.set("user_email", emailUp).set("home_phone", homePhone).set("mobile_phone", mobilePhone)
